  ...
  clickAddToCart(evt) {
    ...
    render() {
      ...
      for (let i = 1; i < avgRating; i++) {
        arr.push(<img src={`/cheerio.png`} key={i} />);
      }
      if (avgRating % 1 > 0.5) {
        arr.push(<img src={'/halfCheerio.png'} key="half" />);
      }
          ...
          <div className="col-xs-9">
            <div className="row">
              <div className="col-xs-6">
                <img className="img-responsive" src={`/cereals/${product.id}.png`} />
              </div>
              <div className="col-xs-6 background-text-box">
                <ul className="product-list">
                <li><strong>{product.title + ' Cheerios'}</strong>
                  {
                    user && user.userType === 'admin'
                    ? <Link to={`/products/${product.id}/editproduct`}>{pencil}</Link>
                    : null
                  }
                </li>
                  <li>${(product.price / 100).toFixed(2)}
                      {
